...
<p style="text-align:center"><strong>A T E N T A M E N T E</strong></p>
    ...
    const savedContent = d.htmlNegacion || localStorage.getItem(storageKey);
        ...
        debounceRef.current = setTimeout(async () => {
            try {
                await import('../api/axios').then(({ default: api }) =>
                    api.patch(`/evaluaciones/${d.id}/html-documento?tipo=negacion`, html, {
                        headers: { 'Content-Type': 'text/plain' },
                    })
                );
            } catch (err) {
                console.error('Error al guardar negación en BD:', err);
            }
        }, 2000);
    }, [d?.id]);
    ...
    const handleRestaurar = useCallback(() => {
        ...
        if (d?.id) {
            import('../api/axios').then(({ default: api }) =>
                api.patch(`/evaluaciones/${d.id}/html-documento?tipo=negacion`, '', {
                    headers: { 'Content-Type': 'text/plain' },
                })
            ).catch(() => {});
        }
